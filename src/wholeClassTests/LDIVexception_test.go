...
/*
 * Tests for ThrowLDIVexception.class. Source code:
 *
...
func initVarsLDIVexception() error {
	...
	_JVM_ARGS = ""
	_TESTCLASS = "ThrowLDIVexception.class" // the class to test
	_APP_ARGS = ""
...
func TestRunLDIVexception(t *testing.T) {
	if testing.Short() { // don't run if running quick tests only. (Used primarily so GitHub doesn't run and bork)
		t.Skip()
	}
	...
	if !strings.Contains(string(slurp), "java.lang.ArithmeticException") &&
		!strings.Contains(string(slurp), "LDIV") {
...
func TestRunLDIVexceptionStrictJDK(t *testing.T) {
	if testing.Short() { // don't run if running quick tests only. (Used primarily so GitHub doesn't run and bork)
		t.Skip()
	}
