...
func TestJdkArrayTypeToJacobinType(t *testing.T) {
    ...
    a := arrays.JdkArrayTypeToJacobinType(arrays.T_BOOLEAN)
    if a != arrays.BYTE {
        t.Errorf("Expected Jacobin type of %d, got: %d", arrays.BYTE, a)
    }

    b := arrays.JdkArrayTypeToJacobinType(arrays.T_CHAR)
    if b != arrays.INT {
        t.Errorf("Expected Jacobin type of %d, got: %d", arrays.INT, b)
    }

    c := arrays.JdkArrayTypeToJacobinType(arrays.T_DOUBLE)
    if c != arrays.FLOAT {
        t.Errorf("Expected Jacobin type of %d, got: %d", arrays.FLOAT, c)
    }

    d := arrays.JdkArrayTypeToJacobinType(999)
    if d != arrays.ERROR {
        t.Errorf("Expected Jacobin type of %d, got: %d", arrays.ERROR, d)
    }
}
...
func TestAaload(t *testing.T) {
    ...
    ptr := pop(&f).(*arrays.JacobinRefArray)
...
func TestAnewrray(t *testing.T) {
    ...
    ptr := element.Value.(*arrays.JacobinRefArray)
    if len(*ptr.Arr) != 13 {
...
func TestByteArrayLength(t *testing.T) {
    f := newFrame(NEWARRAY)
    push(&f, int64(13))                    // make the array 13 elements big
    f.Meth = append(f.Meth, arrays.T_BYTE) // make it an array of bytes
...
func TestIntArrayLength(t *testing.T) {
    ...
    f.Meth = append(f.Meth, arrays.T_INT) // make it an array of ints
...
func TestFloatArrayLength(t *testing.T) {
    ...
    f.Meth = append(f.Meth, arrays.T_DOUBLE) // make it an array of doubles
...
func TestBastore(t *testing.T) {
    ...
    // now, get the reference to the array
    ptr := pop(&f).(*arrays.JacobinByteArray)
    ...
    byteRef := (*arrays.JacobinByteArray)(ptr)
    array := *(byteRef.Arr)
    var sum int64
    for i := 0; i < 30; i++ {
        sum += int64(array[i])
...
func TestCaload(t *testing.T) {
    f := newFrame(NEWARRAY)
    push(&f, int64(30))                    // make the array 30 elements big
    f.Meth = append(f.Meth, arrays.T_CHAR) // make it an array of chars
    ...
    // now, get the reference to the array
    ptr := pop(&f).(*arrays.JacobinIntArray)
...
func TestDaload(t *testing.T) {
    ...
    // now, get the reference to the array
    ptr := pop(&f).(*arrays.JacobinFloatArray)
...
func TestDastore(t *testing.T) {
    ...
    floatRef := (*arrays.JacobinFloatArray)(ptr)
    array := *(floatRef.Arr)
    var sum float64
    for i := 0; i < 30; i++ {
        sum += array[i]
...
func TestFaload(t *testing.T) {
    f := newFrame(NEWARRAY)
    push(&f, int64(30))                     // make the array 30 elements big
    f.Meth = append(f.Meth, arrays.T_FLOAT) // make it an array of floats
...
func TestIastore(t *testing.T) {
    ...
    intRef := (*arrays.JacobinIntArray)(ptr)
    array := *(intRef.Arr)
    var sum int64
    for i := 0; i < 30; i++ {
        sum += array[i]
...
func TestLaload(t *testing.T) {
    f := newFrame(NEWARRAY)
    push(&f, int64(30))                    // make the array 30 elements big
    f.Meth = append(f.Meth, arrays.T_LONG) // make it an array of longs
...
func TestNewrray(t *testing.T) {
    ...
    // now, test the length of the array, which should be 13
    element := g.ArrayAddressList.Front()
    ptr := element.Value.(*arrays.JacobinIntArray)
    if len(*ptr.Arr) != 13 {
