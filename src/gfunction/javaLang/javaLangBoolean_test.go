...
func TestLoad_Lang_Boolean_RegistersMethods(t *testing.T) {
	...
	checks := []struct {
		...
		{"java/lang/Boolean.<init>(Z)V", 1, ghelpers.TrapDeprecated},
		{"java/lang/Boolean.<init>(Ljava/lang/String;)V", 1, ghelpers.TrapDeprecated},
		...
		{"java/lang/Boolean.describeConstable()Ljava.util.Optional;", 0, ghelpers.TrapFunction},
...
func TestBooleanBooleanValue_ValidAndInvalid(t *testing.T) {
	...
	if _, ok := er.(*ghelpers.GErrBlk); !ok {
		t.Fatalf("expected *ghelpers.GErrBlk for wrong field type, got %T", er)
	}
}
