...
func ensureInit() {
	...
	gr.FuncInvokeGFunction = ghelpers.Invoke
...
func TestThreadGroupInitWithParentNameMaxpriorityDaemon_ParamCount(t *testing.T) {
	if _, ok := ThreadGroupInitWithParentNameMaxpriorityDaemon([]any{1}).(*ghelpers.GErrBlk); !ok {
...
func TestThreadGroupInitWithName_AllPaths(t *testing.T) {
	...
	// second not string object
	{
		tgName := "java/lang/ThreadGroup"
		obj := object.MakeEmptyObjectWithClassName(&tgName)
		ok = threadGroupInitWithName([]any{obj, 7}).(*ghelpers.GErrBlk)
		if ok == nil {
...
func TestThreadGroupInitWithParentAndName_AllPaths(t *testing.T) {
	EnsureTGInit()
	// wrong count
	if _, ok := threadGroupInitWithParentAndName([]any{1}).(*ghelpers.GErrBlk); !ok {
		t.Errorf("expected error for wrong count")
	}
	// first not object
	if _, ok := threadGroupInitWithParentAndName([]any{123, object.MakeEmptyObject(), object.StringObjectFromGoString("x")}).(*ghelpers.GErrBlk); !ok {
	...
	// second not object
	{
		tgName := "java/lang/ThreadGroup"
		obj := object.MakeEmptyObjectWithClassName(&tgName)
		if _, ok := threadGroupInitWithParentAndName([]any{obj, 99, object.StringObjectFromGoString("x")}).(*ghelpers.GErrBlk); !ok {
	...
	// third not string object
	{
		...
		obj := object.MakeEmptyObjectWithClassName(&tgName)
		parent := makeThreadGroup("parent")
		if _, ok := threadGroupInitWithParentAndName([]any{obj, parent, 77}).(*ghelpers.GErrBlk); !ok {
	...
	// name null
	{
		...
		obj := object.MakeEmptyObjectWithClassName(&tgName)
		parent := makeThreadGroup("parent")
		if _, ok := threadGroupInitWithParentAndName([]any{obj, parent, object.Null}).(*ghelpers.GErrBlk); !ok {
	...
	// name not a String object
	{
		...
		if _, ok := threadGroupInitWithParentAndName([]any{obj, parent, notString}).(*ghelpers.GErrBlk); !ok {
...
func TestThreadGroupGetName_AllPaths(t *testing.T) {
	// wrong count
	if _, ok := threadGroupGetName([]any{}).(*ghelpers.GErrBlk); !ok {
		t.Errorf("expected error for wrong param count")
	}
	// wrong type
	if _, ok := threadGroupGetName([]any{123}).(*ghelpers.GErrBlk); !ok {
	...
	// invalid type in name field
	{
		...
		if _, ok := threadGroupGetName([]any{obj}).(*ghelpers.GErrBlk); !ok {
