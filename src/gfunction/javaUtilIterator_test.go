...
func TestIterator_Remove_ArrayList(t *testing.T) {
	...
	if _, ok := res.(*GErrBlk); !ok || res.(*GErrBlk).ExceptionType != excNames.IllegalStateException {
...
func TestIterator_Remove_LinkedList(t *testing.T) {
	...
	// Verify s1 is removed
	llst, _ := getLinkedListFromObject(ll)
	if llst.Len() != 1 {
...
func TestIterator_Unsupported(t *testing.T) {
	...
	if _, ok := res.(*GErrBlk); !ok || res.(*GErrBlk).ExceptionType != excNames.UnsupportedOperationException {
