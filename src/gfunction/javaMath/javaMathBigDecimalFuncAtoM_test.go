...
// Helper inside tests: assert ghelpers.GErrBlk with expected exception id
...
func Test_BigDecimal_Divide_Scale_RMode_HalfUp(t *testing.T) {
	...
	if blk, isBlk := res.(*ghelpers.GErrBlk); isBlk {
...
func Test_BigDecimal_Chain_Divide_Subtract_Divide_And_Multiply(t *testing.T) {
	...
	if _, isBlk := v1.(*ghelpers.GErrBlk); isBlk {
	...
	if _, isBlk := v2.(*ghelpers.GErrBlk); isBlk {
