...
// assertGErrBlkMC: small helper for this test file to assert a ghelpers.GErrBlk with a specific exception type
func assertGErrBlkMC(t *testing.T, res interface{}, wantExc int) {
	t.Helper()
	if res == nil {
		t.Fatalf("expected *ghelpers.GErrBlk, got nil")
	}
	blk, ok := res.(*ghelpers.GErrBlk)
	if !ok {
...
func TestLoad_Math_Math_Context_RegistersMethods(t *testing.T) {
	...
	checks := []struct {
		...
		{"java/math/MathContext.<clinit>()V", 0, ghelpers.ClinitGeneric},
