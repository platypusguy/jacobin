...
func Load_Security_MessageDigest() {
	...
	MethodSignatures["java/security/MessageDigest.<clinit>()V"] =
		GMeth{
			ParamSlots: 0,
			GFunction:  clinitGeneric,
		}

	MethodSignatures["java/security/MessageDigest.<init>()V"] =
		GMeth{
			ParamSlots: 0,
			GFunction:  trapProtected,
		}

	MethodSignatures["java/security/MessageDigest.clone()Ljava/lang/Object;"] =
		GMeth{
			ParamSlots: 0,
			GFunction:  msgdigClone,
		}

	MethodSignatures["java/security/MessageDigest.digest()[B"] =
		GMeth{
			ParamSlots: 0,
			GFunction:  msgdigDigest,
		}

	MethodSignatures["java/security/MessageDigest.digest([B)[B"] =
		GMeth{
			ParamSlots: 1,
			GFunction:  msgdigDigestBytes,
		}

	MethodSignatures["java/security/MessageDigest.digest([BII)I"] =
		GMeth{
			ParamSlots: 3,
			GFunction:  msgdigDigestBytesII,
		}

	MethodSignatures["java/security/MessageDigest.engineDigest()[B"] =
		GMeth{
			ParamSlots: 0,
			GFunction:  trapProtected,
		}

	MethodSignatures["java/security/MessageDigest.engineDigest([BII)I"] =
		GMeth{
			ParamSlots: 3,
			GFunction:  trapProtected,
		}

	MethodSignatures["java/security/MessageDigest.engineGetDigestLength()I"] =
		GMeth{
			ParamSlots: 0,
			GFunction:  trapProtected,
		}

	MethodSignatures["java/security/MessageDigest.engineReset()V"] =
		GMeth{
			ParamSlots: 0,
			GFunction:  trapProtected,
		}

	MethodSignatures["java/security/MessageDigest.engineUpdate(B)V"] =
		GMeth{
			ParamSlots: 1,
			GFunction:  trapProtected,
		}

	MethodSignatures["java/security/MessageDigest.engineUpdate([BII)V"] =
		GMeth{
			ParamSlots: 3,
			GFunction:  trapProtected,
		}

	MethodSignatures["java/security/MessageDigest.getAlgorithm()Ljava/lang/String;"] =
		GMeth{
			ParamSlots: 0,
			GFunction:  msgdigGetAlgorithm,
		}

	MethodSignatures["java/security/MessageDigest.getDigestLength()I"] =
		GMeth{
			ParamSlots: 0,
			GFunction:  msgdigGetDigestLength,
		}

	MethodSignatures["java/security/MessageDigest.getInstance(Ljava/lang/String;)Ljava/security/MessageDigest;"] =
		GMeth{
			ParamSlots: 1,
			GFunction:  msgdigGetInstance,
		}

	MethodSignatures["java/security/MessageDigest.getInstance(Ljava/lang/String;Ljava/lang/String;)Ljava/security/MessageDigest;"] =
		GMeth{
			ParamSlots: 2,
			GFunction:  msgdigGetInstanceProvider,
		}

	MethodSignatures["java/security/MessageDigest.getInstance(Ljava/lang/String;Ljava/security/Provider;)Ljava/security/MessageDigest;"] =
		GMeth{
			ParamSlots: 2,
			GFunction:  msgdigGetInstanceProviderObj,
		}

	MethodSignatures["java/security/MessageDigest.getProvider()Ljava/security/Provider;"] =
		GMeth{
			ParamSlots: 0,
			GFunction:  msgdigGetProvider,
		}

	MethodSignatures["java/security/MessageDigest.isEqual([B[B)Z"] =
		GMeth{
			ParamSlots: 2,
			GFunction:  msgdigIsEqual,
		}

	MethodSignatures["java/security/MessageDigest.reset()V"] =
		GMeth{
			ParamSlots: 0,
			GFunction:  msgdigReset,
		}

	MethodSignatures["java/security/MessageDigest.toString()Ljava/lang/String;"] =
		GMeth{
			ParamSlots: 0,
			GFunction:  msgdigToString,
		}

	MethodSignatures["java/security/MessageDigest.update(B)V"] =
		GMeth{
			ParamSlots: 1,
			GFunction:  msgdigUpdateByte,
		}

	MethodSignatures["java/security/MessageDigest.update(Ljava/nio/ByteBuffer;)V"] =
		GMeth{
			ParamSlots: 1,
	...
	MethodSignatures["java/security/MessageDigest.update([BII)V"] =
		GMeth{
			ParamSlots: 3,
...
func msgdigGetInstance(params []any) any {
	...
	md.FieldTable["provider"] = object.Field{
		...
		Fvalue: GetDefaultSecurityProvider(),
	}
...
func msgdigGetInstanceProvider(params []any) any {
	...
	if providerName != types.SecurityProviderName {
		return getGErrBlk(excNames.ProviderNotFoundException,
...
func msgdigGetInstanceProviderObj(params []any) any {
	...
	if providerObj == nil {
		return getGErrBlk(excNames.IllegalArgumentException, "Provider cannot be null")
	}

	// Get provider name from the provider object
	providerNameField, ok := providerObj.FieldTable["name"]
	if !ok {
		return getGErrBlk(excNames.IllegalArgumentException, "Invalid provider object")
	}
...
func msgdigUpdateBytes(params []any) any {
	...
	default:
		return getGErrBlk(excNames.IllegalArgumentException, "Invalid byte array")
	}
...
func msgdigUpdateBytesII(params []any) any {
	...
	if offset < 0 || length < 0 || offset+length > int64(len(bytes)) {
		return getGErrBlk(excNames.IndexOutOfBoundsException, "Invalid offset or length")
	}
...
func msgdigDigestBytesII(params []any) any {
	...
	if offset < 0 || length < int64(digestLen) || offset+int64(digestLen) > int64(len(buf)) {
		return getGErrBlk(excNames.IllegalStateException, "Buffer too small or invalid offset")
	}

	// Compute digest
	digestObj := msgdigDigest([]any{this})
	if errBlk, ok := digestObj.(*GErrBlk); ok {
		return errBlk
	}
