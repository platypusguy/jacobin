...
func TestLoadSecurityProvider(t *testing.T) {
	...
	// Verify clinit
	if _, exists := MethodSignatures["java/security/Provider.<clinit>()V"]; !exists {
		t.Errorf("Expected <clinit>()V method to be registered")
	}

	// Verify constructors
	if _, exists := MethodSignatures["java/security/Provider.<init>(Ljava/lang/String;DLjava/lang/String;)V"]; !exists {
		t.Errorf("Expected deprecated constructor to be registered")
	}

	if _, exists := MethodSignatures["java/security/Provider.<init>(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V"]; !exists {
	...
	for _, method := range expectedMethods {
		if _, exists := MethodSignatures[method]; !exists {
...
func TestLoadSecurityProviderParamSlots(t *testing.T) {
	...
	for _, tt := range tests {
		if gmeth, exists := MethodSignatures[tt.method]; exists {
...
func TestSecurityProviderInitWithInvalidStringVersion(t *testing.T) {
	...
	gerr, ok := result.(*GErrBlk)
	if !ok {
		t.Fatalf("expected *GErrBlk for invalid version, got %T", result)
	}
...
func TestSecurityProviderInitWithInvalidVersionType(t *testing.T) {
	...
	gerr, ok := result.(*GErrBlk)
	if !ok {
		t.Fatalf("expected *GErrBlk for invalid version type, got %T", result)
	}
...
// TestSecurityProviderGetName tests securityProviderGetName
func TestSecurityProviderGetName(t *testing.T) {
	EnsureTGInit()
	...
	result := securityProviderGetName([]any{provider})
	...
	nameStr := object.GoStringFromStringObject(resultObj)
	if nameStr != "MyProvider" {
		t.Errorf("expected 'MyProvider', got '%s'", nameStr)
	}
}

// TestSecurityProviderGetInfo tests securityProviderGetInfo
func TestSecurityProviderGetInfo(t *testing.T) {
	EnsureTGInit()

	className := "java/security/Provider"
	provider := object.MakeEmptyObjectWithClassName(&className)

	infoObj := object.StringObjectFromGoString("Provider Information")
	provider.FieldTable["info"] = object.Field{Ftype: types.StringClassName, Fvalue: infoObj}

	result := securityProviderGetInfo([]any{provider})
	...
	infoStr := object.GoStringFromStringObject(resultObj)
	if infoStr != "Provider Information" {
		t.Errorf("expected 'Provider Information', got '%s'", infoStr)
	}
}
