...
func Load_Util_Zip_Adler32() {
	...
	ghelpers.MethodSignatures["java/util/zip/Adler32.<clinit>()V"] =
		ghelpers.GMeth{
			ParamSlots: 0,
			GFunction:  ghelpers.ClinitGeneric,
		}

	ghelpers.MethodSignatures["java/util/zip/Adler32.<init>()V"] =
		ghelpers.GMeth{
			ParamSlots: 0,
			GFunction:  adlerInit,
		}

	ghelpers.MethodSignatures["java/util/zip/Adler32.getValue()J"] =
		ghelpers.GMeth{
			ParamSlots: 0,
			GFunction:  adlerGetValue,
		}

	ghelpers.MethodSignatures["java/util/zip/Adler32.reset()V"] =
		ghelpers.GMeth{
			ParamSlots: 0,
			GFunction:  adlerReset,
		}

	ghelpers.MethodSignatures["java/util/zip/Adler32.update([BII)V"] =
		ghelpers.GMeth{
			ParamSlots: 3,
			GFunction:  adlerUpdateFromArray,
		}

	ghelpers.MethodSignatures["java/util/zip/Adler32.update(I)V"] =
		ghelpers.GMeth{
			ParamSlots: 1,
			GFunction:  adlerUpdateFromInt,
		}

	ghelpers.MethodSignatures["java/util/zip/Adler32.update(Ljava/nio/ByteBuffer;)V"] =
		ghelpers.GMeth{
			ParamSlots: 1,
