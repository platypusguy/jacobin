...
func assertJavaBoolDate(t *testing.T, got interface{}, want int64, msg string) {
	...
	b, ok := got.(int64)
	if !ok {
		if geb, ok := got.(*ghelpers.GErrBlk); ok {
			t.Fatalf("%s: expected Java boolean, got ghelpers.GErrBlk %d (%s)", msg, geb.ExceptionType, geb.ErrMsg)
		}
...
func TestJavaUtilDate_Init_CurrentTime(t *testing.T) {
	...
	if ret := udateInit([]interface{}{obj}); ret != nil {
		if geb, ok := ret.(*ghelpers.GErrBlk); ok {
...
func TestJavaUtilDate_Deprecated_Constructors_Trap(t *testing.T) {
	...
	for i, ret := range cases {
		geb, ok := ret.(*ghelpers.GErrBlk)
		if !ok {
			t.Fatalf("case %d: expected ghelpers.GErrBlk for deprecated ctor, got %T", i, ret)
		}
