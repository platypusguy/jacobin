...
func TestFilterInputStream_Init_WithFile_Success(t *testing.T) {
	...
	// ghelpers.FilePath copied
	fld, ok := fis.FieldTable[ghelpers.FilePath]
	if !ok {
		t.Fatalf("ghelpers.FilePath not set")
	}
	got := string(object.GoStringFromJavaByteArray(fld.Fvalue.([]types.JavaByte)))
	if got != path {
		t.Fatalf("ghelpers.FilePath mismatch: got %q want %q", got, path)
	}

	// Handle present
	if _, ok := fis.FieldTable[ghelpers.FileHandle].Fvalue.(*os.File); !ok {
		t.Fatalf("ghelpers.FileHandle not *os.File")
	}
	_ = fis.FieldTable[ghelpers.FileHandle].Fvalue.(*os.File).Close()
}
...
func TestFilterInputStream_Init_WithString_Success_And_NotFound(t *testing.T) {
	...
	if _, ok := fis.FieldTable[ghelpers.FileHandle].Fvalue.(*os.File); !ok {
		t.Fatalf("ghelpers.FileHandle not set")
	}
	_ = fis.FieldTable[ghelpers.FileHandle].Fvalue.(*os.File).Close()
...
func TestFilterInputStream_Read_Available_Skip_Close(t *testing.T) {
	...
	obj.FieldTable[ghelpers.FileHandle] = object.Field{Ftype: ghelpers.FileHandle, Fvalue: mustOpenFileFIS(t, path)}
	...
	if geb, ok := inv.(*ghelpers.GErrBlk); !ok || geb.ExceptionType != excNames.IndexOutOfBoundsException {
...
func TestFilterInputStream_Init_FilePathMissing_Error(t *testing.T) {
	...
	if geb, ok := res.(*ghelpers.GErrBlk); !ok || geb.ExceptionType != excNames.IOException {
		t.Fatalf("expected IOException for missing ghelpers.FilePath, got %T", res)
	}
}
