...
func TestLoad_Io_Console_RegistersMethods(t *testing.T) {
	...
	checks := []struct {
		...
		{"java/io/Console.charset()Ljava/nio/charset/Charset;", 0, ghelpers.TrapFunction},
		...
		{"java/io/Console.reader()Ljava/io/Reader;", 0, ghelpers.TrapFunction},
		...
		{"java/io/Console.writer()Ljava/io/PrintWriter;", 0, ghelpers.TrapFunction},
	}
...
func TestConsoleClinit_NoClass_ReturnsErrBlk(t *testing.T) {
	...
	blk, ok := ret.(*ghelpers.GErrBlk)
	if !ok {
		t.Fatalf("expected *ghelpers.GErrBlk, got %T", ret)
	}
