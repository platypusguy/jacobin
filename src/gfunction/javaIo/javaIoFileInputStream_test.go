...
func TestInitFileInputStreamFile_Success(t *testing.T) {
	...
	// Check ghelpers.FilePath copied
	fld, ok := fisObj.FieldTable[ghelpers.FilePath]
	if !ok {
		t.Fatalf("ghelpers.FilePath not set in FileInputStream object")
	}
	gotPath := string(object.GoStringFromJavaByteArray(fld.Fvalue.([]types.JavaByte)))
	if gotPath != path {
		t.Errorf("ghelpers.FilePath mismatch: got %q want %q", gotPath, path)
	}

	// Check ghelpers.FileHandle is set and valid
	fh, ok := fisObj.FieldTable[ghelpers.FileHandle]
	if !ok {
		t.Fatalf("ghelpers.FileHandle not set in FileInputStream object")
	}
	if fh.Ftype != ghelpers.FileHandle {
		t.Errorf("ghelpers.FileHandle field type mismatch: got %v want %v", fh.Ftype, ghelpers.FileHandle)
	}
	fileHandle, ok := fh.Fvalue.(*os.File)
	if !ok {
		t.Fatalf("ghelpers.FileHandle Fvalue is not *os.File")
	}
	fileHandle.Close()
}
...
func TestInitFileInputStreamFile_FilePathMissing(t *testing.T) {
	fisObj := newFileInputStreamObject()
	fileObj := &object.Object{FieldTable: make(map[string]object.Field)} // No ghelpers.FilePath
...
func TestFisReadOne_SuccessAndEOF(t *testing.T) {
	...
	fisObj.FieldTable[ghelpers.FileHandle].Fvalue.(*os.File).Close()
	fld := fisObj.FieldTable[ghelpers.FileHandle]
	fld.Fvalue = mustOpenFile(t, path)
	fisObj.FieldTable[ghelpers.FileHandle] = fld
	...
	err := fisObj.FieldTable[ghelpers.FileHandle].Fvalue.(*os.File).Close()
	if err != nil {
...
func TestFisReadByteArrayOffset_Success(t *testing.T) {
	...
	errObj, ok := resInvalid.(*ghelpers.GErrBlk)
	if !ok {
		t.Fatalf("Expected *ghelpers.GErrBlk error, got %T", resInvalid)
	}
