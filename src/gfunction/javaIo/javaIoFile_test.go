...
func fileCreateThenClose(t *testing.T, params []interface{}) {
	...
	if !ok {
		errMsg := "fileCreateThenClose: File object lacks a ghelpers.FilePath field"
		t.Fatal(errMsg)
	}
...
func TestJavaIoFile_AdditionalGetters(t *testing.T) {
	...
	// Test invalid status
	f.FieldTable[ghelpers.FileStatus] = object.Field{Ftype: types.Int, Fvalue: int64(0)}
...
func TestJavaIoFile_ErrorPaths(t *testing.T) {
	...
	// fileGetPath missing ghelpers.FilePath
	ret := fileGetPath([]interface{}{obj})
	if err, ok := ret.(*ghelpers.GErrBlk); !ok || err.ExceptionType != excNames.IOException {
		t.Fatalf("fileGetPath: expected ghelpers.GErrBlk IOException, got %v", ret)
	}

	// fileIsInvalid missing ghelpers.FileStatus
	ret = fileIsInvalid([]interface{}{obj})
	if err, ok := ret.(*ghelpers.GErrBlk); !ok || err.ExceptionType != excNames.IOException {
		t.Fatalf("fileIsInvalid: expected ghelpers.GErrBlk IOException, got %v", ret)
	}

	// fileDelete missing ghelpers.FilePath
	ret = fileDelete([]interface{}{obj})
	if err, ok := ret.(*ghelpers.GErrBlk); !ok || err.ExceptionType != excNames.IOException {
		t.Fatalf("fileDelete: expected ghelpers.GErrBlk IOException, got %v", ret)
	}

	// fileCreate missing ghelpers.FilePath
	ret = fileCreate([]interface{}{obj})
	if err, ok := ret.(*ghelpers.GErrBlk); !ok || err.ExceptionType != excNames.IOException {
		t.Fatalf("fileCreate: expected ghelpers.GErrBlk IOException, got %v", ret)
	}
}
