...
// helper: make an InputStream-like object that carries ghelpers.FilePath and ghelpers.FileHandle for reading
...
func TestInputStreamReader_Ready_And_Close(t *testing.T) {
	...
	fh := inStreamObj.FieldTable[ghelpers.FileHandle].Fvalue.(*os.File)
	_ = fh.Close()
	...
	fh2 := target.FieldTable[ghelpers.FileHandle].Fvalue.(*os.File)
